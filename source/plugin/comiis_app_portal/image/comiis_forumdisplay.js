function succeedhandle_forum_fav(a, b, c){
	if(b.indexOf("收藏成功") >= 0){
		b = '版块关注成功';
		jQuery('.comiis_forum_fav').attr('href', 'home.php?mod=spacecp&ac=favorite&op=delete&type=forum&formhash='+formhash+'&favid=' + c['favid'] + '&handlekey=forum_fav').text('已关注').removeClass('bg_c f_f').addClass("bg_b f_d");
	}
	popup.open(b, 'alert');
}
...
function succeedhandle_favorite_del(a, b, c){
	...
	jQuery('.comiis_forum_fav').attr('href', 'home.php?mod=spacecp&ac=favorite&type=forum&id=' + c['id'] + '&formhash='+formhash+'&handlekey=forum_fav').text('+ 关注').removeClass('bg_b f_d').addClass("bg_c f_f");
	popup.open(b, 'alert');
}
...
function upzhanlist(tid){
	jQuery.ajax({
		...
		jQuery('#comiis_wxlist_showbox_'+tid).html(s.lastChild.firstChild.nodeValue);
	});
}
var comiis_recommend_key = 0;
function comiis_recommend_addkey(){
	if(jQuery('.comiis_recommend_addkey').length > 0) {
		jQuery('.comiis_recommend_addkey').off('click').on('click', function() {
		if(comiis_recommend_key == 0){
			comiis_recommend_key = 1;
			var comiis_zhankeys = jQuery('.wxlist_bottom_box').length;
			var re_tids = jQuery(this).attr('tid');
			var re_tid = jQuery('.num-all_'+ re_tids);
			var comiis_retidbox = jQuery(this);
			jQuery.ajax({
				type : 'GET',
			...
			.success(function(s) {
				...
				jQuery.ajax({
					...
					if(comiis_zhankeys){
						upzhanlist(re_tids);
						jQuery('#comiis_zhan_key'+ re_tids).text('赞');
					}else{
					...
					jQuery('#comiis_listzhan_' + re_tids + ' i').html('&#xe63b;');
					...
					if(comiis_zhankeys){
						upzhanlist(re_tids);
						jQuery('#comiis_zhan_key'+ re_tids).text('取消');
					}else{
