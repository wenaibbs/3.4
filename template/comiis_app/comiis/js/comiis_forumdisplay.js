function succeedhandle_forum_fav(a, b, c){
	if(b.indexOf("收藏成功") >= 0){
		b = '关注成功';
		$('.comiis_forum_fav').attr('href', 'home.php?mod=spacecp&ac=favorite&op=delete&type=forum&formhash='+formhash+'&favid=' + c['favid'] + '&handlekey=forum_fav').text('已关注').removeClass('bg_a f_f').addClass("bg_b f_d");
	}
	popup.open(b, 'alert');
	upgjlist(c['id']);
}
...
function succeedhandle_favorite_del(a, b, c){
	...
	$('.comiis_forum_fav').attr('href', 'home.php?mod=spacecp&ac=favorite&type=forum&id=' + c['id'] + '&formhash='+formhash+'&handlekey=forum_fav').html('<i class="comiis_font">&#xe610</i>关注').removeClass('bg_b f_d').addClass("bg_a f_f");
	popup.open(b, 'alert');
	upgjlist(c['id']);
}
