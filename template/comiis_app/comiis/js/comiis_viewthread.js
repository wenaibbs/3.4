function succeedhandle_favorite_del(a, b, c){
	...
	$('.comiis_favorites a').attr('href', "home.php?mod=spacecp&ac=favorite&type=forum&id=" + c['id'] + "&formhash=" + formhash + "&handlekey=forum_fav&mobile=2").html('<i class="comiis_font">&#xe610</i>关注');
...
function succeedhandle_forum_fav(a, b, c){
	if(b.indexOf("收藏成功") >= 0){
		...
		if($('#comiis_forum_favtimes em').length > 0){
			...
			$('#comiis_forum_favtimes').html('<em>1</em> 关注');
		}
	}
	popup.open(b, 'alert');
}
...
function comiis_followmod() {
	...
	$.ajax({
		...
		if(s.lastChild.firstChild.nodeValue.indexOf("'type':'add'") > 0){
			obj.text('已关注').attr({"href" : 'home.php?mod=spacecp&ac=follow&op=del&fuid=' + authorid, "class" : "top_nums bg_b f_d y followmod"});
			popup.open('关注成功', 'alert');
		}else if(s.lastChild.firstChild.nodeValue.indexOf("'type':'del'") > 0){
			obj.html('<i class="comiis_font">&#xe610</i>关注').attr({"href" : 'home.php?mod=spacecp&ac=follow&op=add&hash=' + formhash + '&fuid=' + authorid, "class" : "top_nums bg_0 f_f y followmod"});
